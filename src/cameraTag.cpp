...
double tagSolar(){
    //On raspberry 5, the packages are not yet up to date, so you need to manually take a photo to be able to analyse it. 
    ...
    //Calibration matrix (calculated from tests performed)
    ...
    if (markerIds.size() > 0) {
        ...
        for (size_t i = 0; i < markerIds.size(); ++i) {
            if (markerIds[i]==47&&(abs(tvecs[i][0])<abs(dtot))){
                //Data recovery and angle modulation between Pi and -Pi
                a1 = rvecs[i][0];
                a2 = rvecs[i][1];
                a3 = rvecs[i][2];
                d1 = tvecs[i][0];
    ...
    //Linear regression based on tests
    if (a1>a2){
